...
class TagHtmlReportModule(GeneralReportModuleAdapter):
    ...
    def generateReport(self, baseReportDir, progressBar):
        ...
        for sel_tag in self.tags_selected:
            ...
            if len(tags_to_process)>0:
                ...
                self.log(Level.INFO, u"this is a content tags ==> %s <==" % sel_tag)             
        ...
        for sel_tag in self.tags_selected:
            ...
            try:
                if len(sorted_tag)>0:        
                   self.process_thru_tags(report_resources_dir,sorted_tag , tag_number, sel_tag, report_files_dir,progressBar)
    ...
    def process_thru_tags(self, report_dir, tags_to_process, book_mark_number, tag_name, report_files_dir,progressBar):
        ...
        page_file = codecs.open(page_file_name, 'w', self.charEncoding)
        ...
        for tag in tags_to_process:
            ...
            self.create_page_data(page_file, tag, total_tag_number,report_dir, report_files_dir,tag_name,progressBar)
            tag_number = tag_number + 1
    ...
    def create_page_footer(self, page_file, total_pages, current_page, page_file_name,report_dir):
        ...
        try:  
            ...
            footerPath = os.path.join(self.module_dir, "html", "footer.html")
            footerFile = codecs.open(footerPath, 'r', self.charEncoding)            
    ...
    def create_page_data(self, page_file, tag, tag_number,report_dir, report_files_dir, tag_name,progressBar):
        try:
            ...
            exif_dataPath = os.path.join(self.module_dir, "html", 'exif_data.html')
            exif_dataFile = codecs.open(exif_dataPath, 'r', self.charEncoding)
            exif_dataText = exif_dataFile.read()

            gps_dataPath = os.path.join(self.module_dir, "html", 'gps_data.html')
            gps_dataFile = codecs.open(gps_dataPath, 'r', self.charEncoding)               
            ...
            if ("video/" in MimeType):
                ...
                if MimeType in htmlVideoMime:               
                 ...
                 tagType = 'embed' # lub 'iframe'
                 fileType = 'type="application/pdf"'
                 style = 'style="width: 100%; height: 80vh;"' # Dostosuj wysokość jak potrzeba
                 # src zostanie ustawione później na ścieżkę eksportowanego pliku
                 bookmark_file = "bookmark.html" # Upewnij się, że bookmark.html obsługuje embed/iframe  
            
            else:       
                if ("text/" in MimeType or "x-sh" in MimeType ):
                    tagType='iframe' 
            ...
            if not("video/" in MimeType or "image/" in MimeType or "text/" in MimeType or "audio/" in MimeType or "application/x-sh" in MimeType or "application/pdf" in MimeType):         
                #bookmark_file= "bookmark.html"
                ...
                #remove "." if there is no need to change extention 
            ...
            # Subfolder for converted preview files
            preview_dir = os.path.join(report_files_dir, tag_folder_name, "preview")
            if not os.path.exists(preview_dir):
                os.makedirs(preview_dir)
            # Relative HTML path to preview subfolder (from report page to Tagged_Files/tag/preview/)
            ExportedPreviewDir = os.path.join(self.lang.translate("Tagged_Files"), tag_folder_name, "preview")
            ...
            if (MimeType in videoToConvert):
                ...
                self.log(Level.INFO, "Converting to mp4 format")
                if (self.conventer.convert_to_mp4(extractPath, outputFile)):
                    tagType = 'video controls'
                    previewTime = '#t=0.5'
                    src = "..\\" + os.path.join(ExportedPreviewDir, preview_file_name)
                    self.log(Level.INFO, "Converted " + extractPath + " to " + outputFile)
            ...
            if (MimeType in imageToConvert):
                ...
                preview_file_name = exported_file_mame + extention
                outputFile = os.path.join(preview_dir, preview_file_name)
                self.log(Level.INFO, "Converting to jpg format")
                if (self.conventer.convert_to_jpg(extractPath, outputFile)):
                    src = "..\\" + os.path.join(ExportedPreviewDir, preview_file_name)
                    self.log(Level.INFO, "Converted " + extractPath + " to " + outputFile)                      
                        
                        
            
            bookmarkPath = os.path.join(self.module_dir, "html", bookmark_file)
            bookmarkFile = codecs.open(bookmarkPath, 'r', self.charEncoding)               
    ...
    def create_page_header(self, page_file, number_of_tags, tag_name, total_pages,current_page, page_file_name,report_dir,num_of_tags_per_page):
        ...
        try:
            ...
            headerPath = os.path.join(self.module_dir, "html", "header.html")
            headerFile = codecs.open(headerPath, 'r', self.charEncoding)           
    ...
    def create_index_file(self, report_dir):
        ...
        index_file = codecs.open(index_file_name, 'w', self.charEncoding)
    ...
    def create_info_page(self, report_dir):
        ...
        try:  
            ...
            infoPath = os.path.join(self.module_dir, "html", "info.html")
            infoFile = codecs.open(infoPath, 'r', self.charEncoding)           
            ...
            info_file = codecs.open(info_file_name, 'w', self.charEncoding)
            info_file.write(infoText)
    ...
    def create_help_page(self, report_dir):
        ...
        try:  
            ...
            helpPath = os.path.join(self.module_dir, "html", "Help.html")
            helpFile = codecs.open(helpPath, 'r', self.charEncoding)           
            ...
            help_file = codecs.open(help_file_name, 'w', self.charEncoding)
            help_file.write(helpText)
    ...
    def create_menu_file(self, report_dir): 
        ...
        menu_file = codecs.open(menu_file_name, 'w', self.charEncoding)
